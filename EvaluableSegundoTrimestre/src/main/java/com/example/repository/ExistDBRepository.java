...
public class ExistDBRepository {
	...
	public String listado() {
		...
		try {
			...
			// Procesamos los resultados en xml
	...
	public String insertar(ExistJuegoUsuarioDTO usuarioExist, ExistDBService usuario, ExistJuego juego)
			throws Exception {
	...
	private String usuarioToXML(ExistJuegoUsuarioDTO usuarioExist) {
		...
		return "<usuarios><usuario><idSql>" + usuarioExist.getIdSql() + "</idSql><idJuego>" + usuarioExist.getIdJuego()
				+ "</idJuego></usuario></usuarios>";
	}
	...
	private String juegoToXML(ExistJuegoUsuarioDTO usuarioExist, ExistJuego juego) {
		...
		return "<juego><idJuego>" + usuarioExist.getIdJuego() + "</idJuego><titulo>" + juego.getTitulo()
				+ "</titulo><genero>" + juego.getGenero() + "</genero></juego>";
	}
	...
	public String modificarNombre(Long id, String nuevoNombre) throws Exception {
		...
		String sQuery = "update value doc('SegundaEvaluacion/XMLAccesoDatos')/plataforma[@idSql='" + id
				+ "']/nombre with '" + nuevoNombre + "'";
